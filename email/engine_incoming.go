...
func (e *Engine) handlerIncoming(r io.Reader) error {
	...
	if err != nil {
		e.HandlerError(fmt.Errorf("incoming reader error: %s", err))
		return smtp.ErrDataReset
	}
	envelope, err := enmime.ReadEnvelope(bytes.NewReader(body))
	if err != nil {
		e.HandlerError(fmt.Errorf("incoming parser error: %s", err))
		return smtp.ErrDataReset
	}
	...
	if emailTo, err = mail.ParseAddressList(envelope.GetHeader("To")); err != nil {
		e.HandlerError(fmt.Errorf("incoming invalid to header: %s", err))
		return smtp.ErrDataReset
	}
	if emailFrom, err = mail.ParseAddress(envelope.GetHeader("From")); err != nil {
		e.HandlerError(fmt.Errorf("incoming invalid from header: %s", err))
		return smtp.ErrDataReset
	}
	if len(emailTo) > e.IncomingMaxRecipients {
		// SMTP Backend should have filtered this out earlier, but we stop it here jic
		e.HandlerError(fmt.Errorf("incoming too many recipients: %s", err))
		return smtp.ErrDataReset
	}
	...
	if e.IncomingValidateDKIM {
		if _, err := dkim.Verify(bytes.NewReader(body)); err != nil {
			e.HandlerError(fmt.Errorf("incoming invalid dkim signature: %s", err))
			return smtp.ErrDataReset
		}
	}
	...
	for _, mw := range e.incomingMiddleware {
		if proceed, err := mw(email); !proceed {
			if err != nil {
				e.HandlerError(fmt.Errorf("incoming middleware error: %s", err))
			}
			return smtp.ErrDataReset
		}
	}

	// Route to Appropriate Inbox(es)
	receivedBy := 0
	for _, recipient := range emailTo {
		if handler, ok := e.inboxes[recipient.Address]; ok {
			if err := handler(email); err != nil {
				e.HandlerError(fmt.Errorf("incoming inbox handler error: %s", err))
				return smtp.ErrDataReset
			}
			receivedBy++
		}
	...
	if receivedBy == 0 {
		if e.HandlerNoInbox != nil {
			if err := e.HandlerNoInbox(email); err != nil {
				e.HandlerError(fmt.Errorf("incoming no inbox handler error: %s", err))
				return smtp.ErrDataReset
			}
		}
